...
def main():
    ...
    if virtual_env:
        ...
        if venv_path.exists():
            print_colored(f"⚠️  Virtual environment found at {venv_path} but not activated", Colors.YELLOW)
            print("   Run: source venv/bin/activate")
        else:
            print_colored("❌ No virtual environment found", Colors.RED)
            print("   Run: python3 -m venv venv to create one")
