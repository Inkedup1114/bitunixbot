...
func main() {
	var (
		...
		bucketName = flag.String("bucket", "features", "BoltDB bucket name")
	)
	flag.Parse()
	...
	err = db.View(func(tx *bolt.Tx) error {
		...
		if bucket == nil {
			return fmt.Errorf("features bucket not found")
		}
		...
		return bucket.ForEach(func(k, v []byte) error {
			...
			if cutoffTime > 0 && record.Timestamp < cutoffTime {
				return nil // Skip old records
			}
