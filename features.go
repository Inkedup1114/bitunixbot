...
# Configuration
FEATURE_COLS = ['tick_ratio', 'depth_ratio', 'price_dist', 'rolling_volatility', 'bid_ask_spread', 'volume_spike']
LOOKHEAD_SECONDS = 60
...
def load_bolt_data(data_file: str, symbol: str = "BTCUSDT") -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    try:
        ...
        features_df = df[['symbol', 'timestamp', 'tick_ratio', 'depth_ratio', 'price_dist', 
                         'price', 'vwap', 'std_dev', 'bid_vol', 'ask_vol', 'rolling_volatility', 'bid_ask_spread', 'volume_spike']].copy()
...
def create_sample_data(symbol: str, n_samples: int = 10000) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    rolling_volatilities = np.random.normal(0, 1, n_samples)
    bid_ask_spreads = np.random.normal(0, 0.1, n_samples)
    volume_spikes = np.random.normal(0, 50, n_samples)
